...
import { validateUrls, validateFormFactor, groupByMetricAndSort, MAX_URLS } from '../../lib/crux';
...
function createServer(): McpServer {
  ...
  server.tool(
    ...
    {
      urls: z.array(z.string()).min(1, 'At least one URL is required').max(MAX_URLS, `Maximum ${MAX_URLS} URLs allowed`).describe('List of URLs to check'),
    ...
    async ({ urls, formFactor, checkOrigin }: { urls: string[]; formFactor?: string; checkOrigin?: boolean }) => {
      const API_KEY = process.env.PSIKUS;
      if (!API_KEY || typeof API_KEY !== 'string' || API_KEY.trim() === '') {
        return {
          isError: true,
          content: [{ type: 'text' as const, text: 'Missing API key configuration: PSIKUS environment variable is not set or empty' }],
        };
      }
      ...
      try {
        ...
        return {
          isError: true,
          content: [{ type: 'text' as const, text: `Error fetching Crux Data: ${message}` }],
        };
      }
    }
  );
...
export const ALL = async ({ request }: { request: Request }): Promise<Response> => {
